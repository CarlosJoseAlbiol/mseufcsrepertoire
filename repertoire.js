...
async function loadStatuses() {
  try {
    ...
    console.error(err);
    showToast("Could not load statuses: " + err.message, true);
    renderSongs();
  }
}
...
function getMergedSongs(cat, diff) {
  ...
  const custom  = customSongs
    .filter(s => s.category === cat && s.difficulty === diff)
...
function updateStatusButtons(songId, status) {
  ["practiced", "not-practiced", "assessed"].forEach(s => {
    const btn = document.getElementById(`btn-${songId}-${s}`);
    if (!btn) return;
    btn.classList.remove("active-practiced", "active-not-practiced", "active-assessed");
  ...
  if (lbl) {
    ...
    lbl.style.color = status === "practiced"    ? "var(--s-practiced)"
                    : status === "not-practiced" ? "var(--s-not-practiced)"
                    : status === "assessed"       ? "var(--s-assessed)"
                    : "var(--muted)";
  }
}
...
function openAddModalFor(diff) {
  openAddModal();
  document.getElementById("m-diff").value = diff;
}
...
async function saveSongModal() {
  ...
  try {
    if (editId) {
      const { error } = await db.from("repertoire_songs")
        .update({ title, difficulty: diff, solo, category })
        .eq("id", editId);
      if (error) throw error;
      showToast("✅ Song updated!");
    } else {
      const { error } = await db.from("repertoire_songs").insert([{
        id:         "custom_" + Date.now(),
        title,
        difficulty: diff,
        solo,
    ...
    btn.disabled    = false;
    btn.textContent = editId ? "💾 Update Song" : "💾 Save Song";
  }
}
...
function renderSongs() {
  ...
  ["EASY", "MEDIUM", "HARD"].forEach(diff => {
    ...
    const songs = getMergedSongs(currentCat, diff).filter(s => {
      ...
      const matchS = !filterStatus || st === filterStatus;
      ...
      <div class="diff-section">
        ...
        <table class="song-table">
          ...
          <tbody>
            ${songs.map((s, i) => {
              const st  = statuses[s.id] || "none";
              const stC = st === "practiced"    ? "var(--s-practiced)"
                        : st === "not-practiced" ? "var(--s-not-practiced)"
                        : st === "assessed"       ? "var(--s-assessed)"
                        : "var(--muted)";
              return `
                <tr>
                  ...
                  <td class="song-title">
                    ${escHtml(s.title)}
                    ${s.isCustom ? `<span style="font-size:.64rem;color:var(--gold);margin-left:6px;opacity:.75">★</span>` : ""}
                  </td>
                  <td><span class="solo-badge solo-${s.solo}">${s.solo}</span></td>
                  <td>
                    <div class="status-group">
                      <button class="status-toggle ${st === "practiced" ? "active-practiced" : ""}"
                              id="btn-${s.id}-practiced"
                              onclick="setStatus('${s.id}','practiced')" title="Practiced">✔</button>
                      <button class="status-toggle ${st === "not-practiced" ? "active-not-practiced" : ""}"
                              id="btn-${s.id}-not-practiced"
                              onclick="setStatus('${s.id}','not-practiced')" title="Not Practiced">✘</button>
                      <button class="status-toggle ${st === "assessed" ? "active-assessed" : ""}"
                              id="btn-${s.id}-assessed"
                              onclick="setStatus('${s.id}','assessed')" title="Sang for Assessment">◉</button>
                    </div>
                  </td>
                  <td>
                    <span class="status-label" id="lbl-${s.id}" style="color:${stC}">
                      ${st !== "none" ? STATUS_LABELS[st] : ""}
                    </span>
                  </td>
...
function updateStats() {
  let total = 0, practiced = 0, notPracticed = 0, assessed = 0;
  ["EASY", "MEDIUM", "HARD"].forEach(diff => {
    getMergedSongs(currentCat, diff).forEach(s => {
      total++;
      const st = statuses[s.id] || "none";
      if (st === "practiced")     practiced++;
      if (st === "not-practiced") notPracticed++;
      if (st === "assessed")      assessed++;
    });
  });
  const none = total - practiced - notPracticed - assessed;
  document.getElementById("stats-bar").innerHTML = `
    <span class="stat-pill" style="background:rgba(63,185,80,.15);color:var(--s-practiced)">✔ ${practiced}</span>
    <span class="stat-pill" style="background:rgba(248,81,73,.12);color:var(--s-not-practiced)">✘ ${notPracticed}</span>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("song-modal").addEventListener("click", e => {
    if (e.target === document.getElementById("song-modal")) closeModal();
  });
  document.getElementById("delete-modal").addEventListener("click", e => {
    if (e.target === document.getElementById("delete-modal")) closeDeleteModal();
  });
  document.getElementById("m-title").addEventListener("keydown", e => {
    if (e.key === "Enter") saveSongModal();
  });
});
